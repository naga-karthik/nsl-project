...
def save_logs(
    ...
    with open(os.path.join(log_dir, "train_mem.txt"), "w") as f:
        f.write("\n".join(str(item) for item in train_mem))
    ...
    with open(os.path.join(log_dir, "valid_mem.txt"), "w") as f:
        f.write("\n".join(str(item) for item in valid_mem))
